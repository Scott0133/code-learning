...
int height(BTNode *root); // 递归方式查询二叉树高度
int AVL_judge(BTNode *root); // 递归方式判断二叉树是否为平衡二叉树
...
int main()
{
    ...
    MakeTree(&y, 'A', &z, &x); 
    // LevelOrderTree(&y);
    // int height_ret = height(y.root);
    int AVL_ret = AVL_judge(y.root);
    printf("\n%d", AVL_ret);
    return 0;
}
...
int height(BTNode *root) // 递归查询二叉树高度
{
    if (root == NULL) {
        return FALSE;
    }
...
int AVL_judge(BTNode *root)
{
    if (root == NULL) { // 
        return TRUE;
    }
    if (abs(height(root->lChild)-height(root->rChild) <= 1)) { // 用高度差是否超过1判断是否为平衡二叉树
        return AVL_judge(root->lChild) && AVL_judge(root->rChild);
    } else {
        return FALSE;
    }
}